...
, version = Some "0.1.0"
, description = Some
    "Bootstrap a single-package Haskell library in a fresh git repository. Composes `haskell-library` (which pulls in `nix-haskell-flake` to provide the dev shell) with `git-init`; seihou's planner runs all file generation before any commands, so the resulting `git init` + initial commit captures the full scaffold in one shot. When `git.createGithub` is true, git-init will also prompt for `git.repoName` and push to GitHub via `gh repo create` — typically you'd answer with the same value as `project.name`."
, modules =
  [ { module = "haskell-library", vars = [] : List { name : Text, value : Text } }
