...
description: 'Agent-driven, in-place remediation of a repo that already consumes the
  ...
  migrate + seihou run --force) and relocates every local edit made directly to a
  seihou-managed file into its unmanaged, upgrade-safe home — extra dev-shell tools
  in nix/haskell.nix move to flake.module.nix via haskellProject.extraDevPackages,
  custom .envrc exports move to .envrc.local, and extra process-compose processes
  move to process-compose.override.yaml — git-tracking the files Nix needs to see
  and proving dev-shell parity with nix print-dev-env; reviews and verifies but never
  commits'
